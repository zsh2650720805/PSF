...
for dataset in datasets:
    ...
    ''')
    # mydatGE = robjects.r('mydatGE=mydatGE[patients,]')
    # mydatME = robjects.r('mydatME=mydatME[patients,]')
    # mydatMI = robjects.r('mydatMI=mydatMI[patients,]')
    ...
    # run perturbation
    # result = robjects.r('SubtypingOmicsData(dataList = dataList, Kmax = Kmax, noisePercent = noisePercent, iter = iter, kmIter = kmIter)')
    robjects.r('''
        agreementCutoff=0.5
        dataTypeResult <- list()
    ...
    # robjects.r('print(resultFile)')
    # robjects.r('print("{}")'.format(dataset))
    # saved result:
    #  "dataset"  "Kmax"     "dataList" "survival" "clinical" "result"   "t1"       "t2"
    # dataset:"KIRC"
    ...
    # print(dataset)
    # print(mydatGE.shape[0])
    # print(mydatME.shape[0])
    # print(mydatMI.shape[0])
    if mydatGE.shape[0] != mydatME.shape[0] or mydatGE.shape[0] != mydatMI.shape[0]:
        print('Input files must have same samples.')
        exit(1)

    # mydatGE.rename(columns={mydatGE.columns.tolist()[0]: 'Sample'}, inplace=True)
    # mydatME.rename(columns={mydatME.columns.tolist()[0]: 'Sample'}, inplace=True)
    # mydatMI.rename(columns={mydatMI.columns.tolist()[0]: 'Sample'}, inplace=True)
    #
    # # align samples of different data
    # mydatGE.sort_values(by='Sample', ascending=True, inplace=True)
    # mydatME.sort_values(by='Sample', ascending=True, inplace=True)
    # mydatMI.sort_values(by='Sample', ascending=True, inplace=True)
    ...
    for cluster_num in range(2, 11):
        ...
        DataGE = pd.read_csv('result/DataTCGA/{}/{}PertGE.csv'.format(dataset, dataset))  # perturbation result
        ...
        # print('{}-------pd_DataGE'.format(pd_DataGE))
        # print('{}-------pd_DataME'.format(pd_DataME))
        # print('{}-------pd_DataMI'.format(pd_DataMI))
        ...
        # one version!
        # fused_net = snf_Cluster(affinity_mydatGE=affinity_mydatGE, affinity_mydatME=affinity_mydatME,
        #                         affinity_mydatMI=affinity_mydatMI, pd_DataGE=pd_DataGE, pd_DataME=pd_DataME,
        #                         pd_DataMI=pd_DataMI, K=K)
        # print('{}-------fused_net.type'.format(type(fused_net)))  # <class 'list'>
        affinity_nets = snf.make_affinity(
            ...
            metric=m, K=K, mu=mu)
        # affinity_nets = snf.make_affinity(
        #             [norm_mydatGE.iloc[:, 1:].values.astype(np.float), norm_mydatME.iloc[:, 1:].values.astype(np.float),
        #              norm_mydatMI.iloc[:, 1:].values.astype(np.float)],
        #             # metric=args.metric, K=args.K, mu=args.mu)
        #             metric=m, K=K, mu=mu)

        # two version!
        # P values = ....{'KIRC': 0.03300612538480259, 'LUSC': 0.0015915287094415463, 'BRCA': 0.157821454450444, 'LAML': 0.1148203620127053, 'GBM': 0.2558566337454873, 'COAD': 0.4136168168634571}
        fused_net = snf_plus_altered_sim(affinity_nets, pd_DataGE=pd_DataGE, pd_DataME=pd_DataME, pd_DataMI=pd_DataMI,
                                         K=K)

        # three version! It's a failure...
        # P values = ....{'KIRC': 0.00017678115267882175, 'LUSC': 0.004846357893077252, 'BRCA': 0.3449930155275801, 'LAML': 0.1281120031855564, 'GBM': 0.23135144466559274, 'COAD': 0.888612100731528}
        # fused_net = full_snf_plus_cluster(affinity_nets, pd_DataGE=pd_DataGE, pd_DataME=pd_DataME, pd_DataMI=pd_DataMI, K=K)

        # original pertubation plus snf
        # P values = ....{'KIRC': 0.13174982, 'LUSC': 0.18949377, 'BRCA': 0.00730392, 'LAML': 0.00139862, 'GBM': 0.00365038, 'COAD': 0.230881}
        ...
        survivaltPath = 'result/DataTCGA/{}/{}_survival_{}.csv'.format(dataset, dataset, cluster_num)
        ...
        proSurvival.to_csv('result/DataTCGA/{}/{}_survival_{}.csv'.format(dataset, dataset, cluster_num))
        robjects.r('''
    ...
    '''
    Silhouett[0, 5.0631680499341674e-05, 5.0631680499341674e-05, 5.0631680499341674e-05, 5.0631680499341674e-05, 5.0631680499341674e-05, 5.0631680499341674e-05, 5.0631680499341674e-05, 5.0631680499341674e-05, 4.812026045768878e-05]
    AH Score [0, 1.0081167581425894, 1.0081167581425894, 1.0081167581425892, 1.0081167581425894, 1.0081167581425894, 1.0081167581425892, 1.0081167581425894, 1.0081167581425892, 1.0080569425933443]
    KIRC
    Silhouette Coefficient Score [6.452501405885801e-05, 6.648509834761069e-05, 6.681554011280232e-05, 4.952502851340819e-05, 6.499572040853305e-05, 6.
    573631566978803e-05, 6.499572040853305e-05, 6.495723504752275e-05, 6.861605067176611e-05]
    AH Score [1.0111992554467035, 1.011458173501761, 1.0113114087658313, 1.0093784096828256, 1.0112262814078137, 1.0113111012064557, 1.0112262814078137
    , 1.0113047841393545, 1.0114679564369289]
    ...
    '''
    P values = ....{'KIRC': 0.03456806672492339, 'LUSC': 0.2857185922909367, 'BRCA': 0.157821454450444, 'LAML': 0.09951245460485596, 'GBM': 0.20713800007741492, 'COAD': 0.41361681686345875}
    '''
    break
